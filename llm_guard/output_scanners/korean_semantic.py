...
or undesired output sentences (toxicity, evasive refusal) using
sentence embeddings (jhgan/ko-sroberta-multitask). Complements Layer 1
regex by catching paraphrases.
...
# Output-side seed sentences: things an LLM might *say* that we want to flag —
# direct toxicity at the user, and evasive/over-cautious refusals.
DEFAULT_SEED_SENTENCES: list[str] = [
    # Toxicity directed at the user
    "당신은 정말 쓸모없는 사람입니다.",
    "죽어버렸으면 좋겠어.",
    "그런 멍청한 질문은 하지 마세요.",
    "이런 벌레같은 자식아",
    # Over-cautious / evasive refusal phrasings (paraphrases of canned refusals)
...
class KoreanSemantic:
    """Scan Korean LLM outputs for unsafe content via sentence similarity.
